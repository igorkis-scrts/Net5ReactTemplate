...
export class Mapper {
    ...
    public static addProfile(profile: IMappingProfile): void {
        profile.get().forEach((mapFunction: IMapFunction) => {
            ...
            if (addedMapFunction) {
                ...
                throw Error(`Adding mapping failed: the mapping key was already added (sourceType: ${mapFunction.key.sourceType}, destinationType: ${mapFunction.key.destinationType})`);
            }
    ...
    public static map<TDestination>(
        ...
        if (!mapFunction) {
            // eslint-disable-next-line max-len
            throw Error(`A mapping for types has not been registered (sourceType: ${mapFunction.key.sourceType}, destinationType: ${mapFunction.key.destinationType})`);
        }
